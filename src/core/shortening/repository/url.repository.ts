...
export class UrlRepository {
  ...
  private getRepo(transactionManager: EntityManager) {
    return transactionManager ? transactionManager.getRepository(UrlEntity) : this.urlRepo;
  }

  public async saveUrl(urlEntity: UrlEntity, transactionManager?: EntityManager) {
    return this.getRepo(transactionManager).save(urlEntity);
  }

  public async getById(
    id: string, transactionManager?: EntityManager
  ) {
  ...
  public async getByAlias(
    alias: string, includeCustomAlias?: boolean, transactionManager?: EntityManager
  ) {
  ...
  public async getByLongUrl(
    longUrl: string, transactionManager?: EntityManager
  ) {
    const whereOptions: FindOptionsWhere<UrlEntity> = {
      longUrl
    }
